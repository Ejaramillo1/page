...
```{r}
knitr::include_graphics("bill_idx.JPG")
```

El índice de billonarios de bloomberg nos ofrece datos sobre las personas más adineradas del mundo. En total se contabilizan 500 personas pero, por ejemplo, si quisieramos saber cuantos son por país, cuantos son por industria u otro dato pues los tenemos que contar uno por uno. Al ser sólo una tabla (-.-) <-- _carita de weva_ no nos ofrece nada divertido para visualizar. Nomás por cura nosotros vamos a hacer un breve análisis de la tabla. Lo primero que queremos saber es cuál es el país con más billonarios del mundo **_SPOILER ALERT!!!_** es **ESTADOS UNIDOS**.
...
Para conseguir los datos, podríamos hacer un simple copy-paste de tooooooda la tabla y luego pegarla en excel pero la neta **QUE WEVA** además, cada vez que necesitemos traer los datos tenemos que hacer la misma operación (copiar y pegar los 500 registros a mano) osea **MAS QUE WEVA**.Por esto, haremos uso de la magia de las computadoras, el conocimiento milenario de los programadores que crearon el paquete de funciones **rvest**, y mucho internet para traer la tabla.

Lo chido de esto es que la computadora se comunica con la página web cada vez que se hace la operación entonces, uno sólo tiene que picar el botoncito para que se baje la info solita. 
