...
La base de datos del Titanic es uno de los primeros pasos para adentrarse en el mundo de la ciencia de datos desde el aprendizaje en esta página **Kaggle** con muchos recursos a la mano. El objetivo de la base de datos y la competencia es tratar de estimar si un pasajero sobrevive o no y su probabilidad.
Debido a que un análisis de este tipo es relativamente largo a pesar de que la base de datos sea pequeña lo dividimos en tres partes. La primera comprende una primera exploración de las variables que no tienen valores perdidos y la creación de nuevas variables a partir de las anteriores, la segunda parte será completar los valores perdidos de las variables incompletas. La última parte será la estimación y validación del modelo a partir del cual se estimará la probabilidad de supervivencia de un pasajero. 
Como en todos los ejercicios de análisis y estimación en **R** primero vamos a cargar los paquetes de funciones que se utilizarán en el ejercicio de estimación.
...
Posteriormente vamos a subir las tablas de datos con que trabajaremos.

```{r warning=FALSE, message=FALSE}

train <- read_csv("C:/Users/ejaramillo/Documents/titanic_competition_kaggle/train.csv") 
test <- read_csv("C:/Users/ejaramillo/Documents/titanic_competition_kaggle/test.csv")


```

Para no tener que hacer el proceso de limpieza dos veces vamos a unir las bases de datos traen and test, así tendremos las dos tablas con los datos normalizados al momento de la estimación.
...
Vamos a echar una ojeada a la tabla de datos para saber cómo están compuestas las variables.
...
Como podemos ver arriba, en nuestra tabla de datos está compuesta por doce variables de las cuales **Fare**, **Age** y **PassengerId** son variables numéricas, las restantes 9 variables más son categóricas sin embargo la variable **Pclass** se leyó como o variable numérica por lo que debemos recodificar como variable de tipo factor para obtener resultados satisfactorios. 
Una de las tareas principales al realizar análisis estadístico es conocer la base de datos y las relaciones entre las variables, así como conocer los valores perdidos. Para esto aplicaremos una función que nos ayudará a contar los valores perdidos en cada una de las columnas en nuestra tabla de datos.
...
Confirmamos con una función a la medida y de manera gráfica los valores perdidos en las variables.
...
Las variables con mayor número de valores perdidos son la edad, la cabina y el lugar en el que se embarcaron, por otro lado, la variable **Survived** muestra una gran cantidad de valores perdidos, esto se da principalmente porque unimos la tabla de datos de entrenamiento y de modelado.
## Exploración de la variable respuesta: Survived
Iniciamos la exploración de la tabla de datos analizando el comportamiento de la variable respuesta, en este caso de trata de una variable dicotómica que indica si la persona en cuestión sobrevivió o no.
...
Por ejemplo, vamos a comparar las barras que se encuentran encima del número cero de color azul, o sea hombres que no sobrevivieron. 
...
Del total de hombres que no sobrevivieron 300 viajaban en tercera clase, 91 en segunda clase y 77 en primera clase. Por lo que podemos inferir que debido a la gran diferencia entre las variables éste puede ser un predictor importante.
...
Ahora que sucedió con las mujeres que no sobrevivieron de acuerdo con la clase en la que viajaban.
...
Por otro lado, los hombres que menos sobrevivieron son los que se encontraban en segunda clase, siendo esto diferente para los de tercera y primera clase, sin embargo, no sabemos si esto es natural hablando en porcentajes en función del número de pasajeros del mismo sexo por clase. 
...
EdadLM <- lm(Age ~ pr + title + Fsize  + Fare, data = all[!is.na(all$Age), ])
summary(EdadLM)
...
Si observamos detenidamente el la gráfica color negro podemos ver que la 11va variable es **ageLM**, una estimación de la edad para los valores perdidos de la variable **Age**

En el siguiente blog continuaremos con la exploración de la base de datos y probablemente realizaremos las estimaciones de la probabilidad de supervivencia en función de las variables creadas en esta parte.

Por lo pronto terminamos con éste análisis.
